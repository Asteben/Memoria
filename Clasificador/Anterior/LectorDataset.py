# run as: python 3 read_jsonl.py data/example.jsonl corpus/example.corpus
...
def ReadJsonl( raw_tweets_file_name , corpus_file_name ):
    ...
    for line in raw_tweets_file :
        ...
        tweet = [ word.lower () for word in tweet if not ("@" in word or "http" in word or "#" in word or "" == word or " " == word)]
...
def main () :
    if len(sys.argv) < 3:
        print ( "correct usage: python 3 read_jsonl.py <input_raw_tweets_file_name> <output_corpus_file_name>")
        return
