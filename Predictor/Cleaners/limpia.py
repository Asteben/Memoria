...
with open('../../Data/rawdataset/12MCoronaTweets.csv', 'r', encoding="utf-8") as csv_obj:
    ...
    rowCount = 0
    rowCountClean = 0
    with open('../../Data/rawdataset/CoronaTweetsLimpio3.csv', 'w', encoding="utf-8", newline='') as csv_obj_w:
        csvf = DictWriter(csv_obj_w, fieldnames = ['id', 'created_at', 'text'])
        ...
        for  row in csv:
            ...
            if "RT" in row['text']:
                ...
                csvf.writerow({'id':rowCountClean, 'created_at':UnixTime, 'text': row['text']})
